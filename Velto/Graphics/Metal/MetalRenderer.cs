...
public unsafe class MetalRenderer : IRenderer
{
    ...
    private List<MetalTexture> textures = new();
    ...
    {
        var drawable = this.device.MetalLayer.NextDrawable;
        ...
        foreach (var set in DrawCallSets)
        {
            ...
            {
                encoder.UseResource(texture.Handle, MTLResourceUsage.Read, MTLRenderStages.RenderStageFragment);
            }
    ...
    public void DrawTexture(ITexture texture, Vector2 position, Vector2 size, Color4<Rgba> color, float rotation = 0)
    {
        ...
        textures.Add(texture as MetalTexture);
    }
